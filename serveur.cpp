...
void Serveur::chargerRegion(const string& nomRegion) {
    ...
    while (getline(fichier, ligne)) {
        ...
        for (size_t i = 1; i < colonnes.size(); ++i) {
            ...
            if (!typeRoute.empty()) {
                // On normalise en minuscule pour la Fabrique (si votre fabrique attend des minuscules)
                // Sinon, assurez-vous que la Fabrique gère "Autoroute" avec majuscule.
                ...
                // Debug optionnel
                cout << "Route connue: " << villeDepart << " <-> " << villeArrivee << " : " << typeRoute << endl;
            }
        }
    }
...
bool Serveur::recevoirDonnees(SocketType socket, char* buffer, int tailleAttendue) {
    ...
    while (totalRecu < tailleAttendue) {
        ...
        if (recu <= 0) return false; // Erreur ou déconnexion
        totalRecu += recu;
    }
    return true;
}
...
bool Serveur::envoyerDonnees(SocketType socket, const string& message) {
    ...
    if (send(socket, (char*)&tailleReseau, sizeof(int), 0) == SOCKET_ERROR) return false;

    // 2. Envoi du contenu
    int totalEnvoye = 0;
    while (totalEnvoye < tailleLocale) {
        int envoye = send(socket, message.c_str() + totalEnvoye, tailleLocale - totalEnvoye, 0);
        if (envoye == SOCKET_ERROR) return false;
        totalEnvoye += envoye;
    }
    return true;
}
...
void Serveur::gererClient(SocketType clientSocket) {
    ...
    cout << "[Client] JSON recu : " << jsonRecu << endl;
    ...
    try {
        ...
        // C'est ici qu'on utilise votre EXCEL simulé
        for (size_t i = 0; i < sommets.size(); ++i) {
            for (size_t j = i + 1; j < sommets.size(); ++j) {
                ...
                string typeRoute = "nationale"; // Défaut

                if (baseDeDonneesRoutes.count(clef)) typeRoute = baseDeDonneesRoutes[clef];
                else if (baseDeDonneesRoutes.count(clefInv)) typeRoute = baseDeDonneesRoutes[clefInv];
                else if (dist > 100) typeRoute = "autoroute"; // Logique par défaut
        ...
        // === 3. RESOLUTION TSP ===
        // Note: Assurez-vous que sommets[0] est valide comme point de départ
        auto tournee = AlgorithmeTSP<shared_ptr<Route>, Ville, EvaluateurRoutePtr>::resoudre(carte, sommets[0]);
        
        // Simulation résultat pour l'exemple :
        // auto tournee = sommets; // Juste pour que ça compile, mettez le vrai algo

        // === 4. PREPARATION REPONSE JSON ===
        json jOutput = json::array();
        for (auto* s : tournee) {
            json villeObj;
            villeObj["ville"] = s->v.getNom();
            villeObj["latitude"] = s->v.getLatitude();
            villeObj["longitude"] = s->v.getLongitude();
            // On peut ajouter des infos sur l'arête empruntée pour y arriver si besoin
            jOutput.push_back(villeObj);
        }

        string reponse = jOutput.dump(); // Convertit en string
